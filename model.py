    ...
    def forward(self, x, stochastic_flag=True, k=1):
        if stochastic_flag:
            ...
            epsilon_weights = torch.randn(
                k, self.input_dim, self.output_dim, device=self.device
            )
...
class MLPDecoder(nn.Module):
    def __init__(self, latent_dim, y_num_dim, decode_dim, p_drop=0.0, device="cpu"):
        ...
        self.fc1 = nn.Linear(latent_dim, decode_dim)
        self.dropout1 = nn.Dropout(p_drop)
        self.fc2 = nn.Linear(decode_dim, y_num_dim)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if type == "SE" or type == "PROD":
            ...
            self.scale_prior_std = torch.tensor([1.0], device=self.device)
            self.scale_posterior_mean = nn.Parameter(
                torch.tensor([se_scales] * dim, device=self.device)
            )
            self.scale_posterior_log_std = nn.Parameter(
                torch.tensor([np.log(1)] * dim, device=self.device)
            )

            self.alpha_prior_mean = torch.tensor([0], device=self.device)
            self.alpha_prior_std = torch.tensor([1.0], device=self.device)
            self.alpha_posterior_mean = nn.Parameter(
                torch.tensor([se_alphas] * dim, device=self.device)
            )
            self.alpha_posterior_log_std = nn.Parameter(
                torch.tensor([np.log(1)] * dim, device=self.device)
            )
        ...
        if basis_func == "regff" or basis_func == "hs":
            self.omega = torch.arange(0, self.M, device=self.device).unsqueeze(0)
            self.J = float(2.55)
        else:
            pass
        ...
        if self.type == "SE" or self.type == "PROD":
            ...
            if stochastic_flag:
                if self.scale_posterior_mean.requires_grad:
                    scales = (
                        torch.randn(self.dim, device=self.device) * scale_posterior_std
                ...
                if self.alpha_posterior_log_std.requires_grad:
                    alphas = (
                        torch.randn(self.dim, device=self.device) * alpha_posterior_std
...
class DGBFGP(nn.Module):
                ...
                covar_info.update(
                    {
                        ...
                        "A": BayesianLinear(
                            id_embed_dim, latent_dim, device=self.device
                        ),
    ...
    def klx_loss(self, densities):
        ...
        KL = sum(kls, start=torch.zeros(1, device=self.device))
        return KL
